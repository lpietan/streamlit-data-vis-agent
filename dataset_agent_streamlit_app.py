...
# Helper to get writable data directory (Docker: /app/data, Cloud: fallback to temp dir)
def get_data_dir():
    ...
    try:
        ...
        testfile = os.path.join(docker_data_dir, 'test_write.tmp')
...
def cleanup_generated_files():
    ...
    files_to_clean = [
        os.path.join(data_dir, 'generated_code.py'),
        os.path.join(data_dir, 'output_plot.png'),
        os.path.join(data_dir, 'transformed_data.csv'),
        os.path.join(data_dir, 'transformed_data.xlsx'),
        os.path.join(data_dir, 'transformed_data.json')
    ]
    for file in files_to_clean:
        if os.path.exists(file):
            try:
                ...
                st.warning(f"Could not remove file {file}: {str(e)}")
...
class DatasetAgent:
    def __init__(self, llm):
        ...
        self.default_output_names = {
            'plot': os.path.join(data_dir, 'output_plot.png'),
            'data': os.path.join(data_dir, 'transformed_data.csv'),
            'excel': os.path.join(data_dir, 'transformed_data.xlsx'),
            'json': os.path.join(data_dir, 'transformed_data.json')
        }
...
4. For any outputs:
   - Plots/graphs save to '{os.path.join(data_dir, 'output_plot.png')}'
   - save figures with high resolution and without empty space (dpi=300, bbox_inches='tight')
   - Transformed data save to '{os.path.join(data_dir, 'transformed_data.csv')}'
...
Return only the Python code without any additional text or markdown."""
    ...
    def get_code(self, file_path: str, query: str):
        try:
            ...
            data_dir = get_data_dir()
            with open(os.path.join(data_dir, 'generated_code.py'), 'w') as file:
                file.write(result)
...
# Cleanup temp files (input dataset and code temp files)
def cleanup_temp_files():
    data_dir = get_data_dir()
    for fname in os.listdir(data_dir):
        if fname.startswith('tmp') and (fname.endswith('.csv') or fname.endswith('.py')):
            try:
                os.remove(os.path.join(data_dir, fname))
            except Exception as e:
                st.warning(f"Could not remove temp file {fname}: {str(e)}")
...
def main():
    ...
    if st.button("Submit", type="primary", use_container_width=True):
        ...
        data_dir = get_data_dir()
        with tempfile.NamedTemporaryFile(delete=False, suffix='.csv', dir=data_dir) as tmp_file:
        ...
        try:
            ...
            if env == 'cloud':
                ...
                # To enable, run: docker run --rm -v /app/data:/app/data python:3.12-slim python /app/data/<filename>
                result = subprocess.run([
                    "docker", "run", "--rm",
                    "-v", f"{data_dir}:{data_dir}",
                    "python:3.12-slim",
                    "python", f"{os.path.join(data_dir, os.path.basename(code_file_path))}"
    ...
    # Clean up temp files before zipping outputs
    cleanup_temp_files()
    ...
    if st.session_state.generated_code:
        ...
        data_dir = get_data_dir()
        if os.path.join(data_dir, 'output_plot.png') in st.session_state.generated_files:
            st.image(os.path.join(data_dir, 'output_plot.png'), caption='Generated Figure', use_container_width=True)
        # Show transformed data preview if exists 
        if os.path.join(data_dir, 'transformed_data.csv') in st.session_state.generated_files:
            try:
                df_out = pd.read_csv(os.path.join(data_dir, 'transformed_data.csv'))
