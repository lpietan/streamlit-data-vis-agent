...
def cleanup_generated_files():
    files_to_clean = [
        '/app/data/generated_code.py',
        '/app/data/output_plot.png',
        '/app/data/transformed_data.csv',
        '/app/data/transformed_data.xlsx',
        '/app/data/transformed_data.json'
    ]
...
class DatasetAgent:
    def __init__(self, llm):
        ...
        self.default_output_names = {
            'plot': '/app/data/output_plot.png',
            'data': '/app/data/transformed_data.csv',
            'excel': '/app/data/transformed_data.xlsx',
            'json': '/app/data/transformed_data.json'
        }
...
4. For any outputs:
   - Plots/graphs save to '/app/data/output_plot.png'
   - save figures with high resolution and without empty space (dpi=300, bbox_inches='tight')
   - Transformed data save to '/app/data/transformed_data.csv'
...
Return only the Python code without any additional text or markdown."""
    ...
    def get_code(self, file_path: str, query: str):
        try:
            ...
            with open('/app/data/generated_code.py', 'w') as file:
                file.write(result)
...
def cleanup_temp_files():
    for fname in os.listdir('/app/data'):
        if fname.startswith('tmp') and (fname.endswith('.csv') or fname.endswith('.py')):
            try:
                os.remove(os.path.join('/app/data', fname))
            except Exception as e:
...
def main():
    ...
    if st.button("Submit", type="primary", use_container_width=True):
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.csv', dir='/app/data') as tmp_file:
        ...
        try:
            if env == 'cloud':
                ...
                with tempfile.NamedTemporaryFile(delete=False, suffix='.py', dir='/app/data') as code_file:
                ...
                result = subprocess.run([
                    ...
                    "-v", "/app/data:/app/data",
                    "python:3.12-slim",
                    "python", f"/app/data/{os.path.basename(code_file_path)}"
    ...
    if st.session_state.generated_code:
        ...
        if '/app/data/output_plot.png' in st.session_state.generated_files:
            st.image('/app/data/output_plot.png', caption='Generated Figure', use_container_width=True)
        # Show transformed data preview if exists 
        if '/app/data/transformed_data.csv' in st.session_state.generated_files:
            try:
                df_out = pd.read_csv('/app/data/transformed_data.csv')
        ...
        if st.session_state.generated_files:
            with tempfile.TemporaryDirectory(dir='/app/data') as tmpdir:
